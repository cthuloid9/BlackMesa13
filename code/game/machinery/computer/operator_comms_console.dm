...
/obj/machinery/computer/operator_communications
	name = "operator communications console"
	desc = "This can be used for various important functions. This model has voice synthesis capabilities."
	icon_screen = "operator_communications"
...
/obj/machinery/computer/operator_communications/New()
	shuttle_caller_list += src
	..()

/obj/machinery/computer/operator_communications/process()
	if(..())
		if(state != STATE_STATUSDISPLAY)
			src.updateDialog()


/obj/machinery/computer/operator_communications/Topic(href, href_list)
	if(..())
		return
	...
	switch(href_list["operation"])
		...
		if("vox_announce")
			if(src.authenticated==2)
				if(vox_cooldown)
					usr << "Please allow at least 30 seconds to pass between vox announcements"
					return
				var/temp = input(usr, "Enter an announcement.", "Announcement", last_vox_message) as text
				if(!temp || !(usr in view(1,src)))
					return
				vox_message = temp
				last_vox_message = vox_message

				say_vox_announcement(vox_message)
				vox_cooldown = 1
				icon_screen = "operator_communications_wait"
				update_icon()
...
/obj/machinery/computer/operator_communications/attackby(obj/I, mob/user, params)
	if(istype(I, /obj/item/weapon/card/id))
		attack_hand(user)
	else
		return ..()

/obj/machinery/computer/operator_communications/emag_act(mob/user)
	if(!emagged)
		src.emagged = 1
		if(authenticated == 1)
			authenticated = 2
...
/obj/machinery/computer/operator_communications/attack_hand(mob/user)
	if(..())
		return
	if (src.z > 6)
	...
	var/datum/browser/popup = new(user, "communications", "Operator Communications Console", 400, 500)
		...
		if(STATE_DEFAULT)
			if (src.authenticated)
				...
				if (src.authenticated==2)
					...
					dat += "<BR>\[ <A HREF='?src=\ref[src];operation=vox_announce'>Make a VOX Announcement</A> \]"
					dat += "<BR>\[ <A HREF='?src=\ref[src];operation=vox_help'>View VOX Announcement System Manual</A> \]"
					if(cross_allowed)
...
/obj/machinery/computer/operator_communications/proc/get_javascript_header(form_id)
...
/obj/machinery/computer/operator_communications/proc/get_call_shuttle_form(ai_interface = 0)
...
/obj/machinery/computer/operator_communications/proc/get_cancel_shuttle_form()
...
/obj/machinery/computer/operator_communications/proc/interact_ai(mob/living/silicon/ai/user)
	var/dat = ""
	switch(src.aistate)
		if(STATE_DEFAULT)
			if(SSshuttle.emergencyLastCallLoc)
...
/obj/machinery/computer/operator_communications/proc/make_announcement(mob/living/user, is_silicon)
...
/obj/machinery/computer/operator_communications/proc/post_status(command, data1, data2)
...
/obj/machinery/computer/operator_communications/Destroy()
	shuttle_caller_list -= src
	SSshuttle.autoEvac()
	return ..()

/obj/machinery/computer/operator_communications/proc/overrideCooldown()
	var/obj/item/weapon/circuitboard/computer/communications/CM = circuit
	CM.lastTimeUsed = 0